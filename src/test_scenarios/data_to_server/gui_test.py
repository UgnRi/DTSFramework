...
class DataToServerGUITest(BaseGUITest):
    def __init__(self, device_config: Dict[str, Any], page, scenario_config=None):
        super().__init__(
            device_config=device_config, scenario_name="data_to_server", page=page
        )
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt == max_retries - 1:
                        raise
                    self.logger.warning(
                        f"Navigation attempt {attempt + 1} failed: {str(e)}"
                    )
    ...
    async def execute(self):
        ...
        try:
            ...
            config = {
                ...
                "server_config": {
                    "server_address": scenario_config["server_config"][
                        "server_address"
                    ],
                    ...
                    "enable_secure_connection": scenario_config["server_config"].get(
                        "enable_secure_connection", False
                    ),
                    "use_credentials": scenario_config["server_config"].get(
            ...
            if "collection_config" in scenario_config:
                if "day_time" in scenario_config["collection_config"]:
                    ...
                    config["collection_config-scheduler"] = {
                        ...
                        "interval_type": scenario_config["collection_config"][
                            "interval_type"
                        ],
                        "month_day": scenario_config["collection_config"]["month_day"],
                        "weekdays": scenario_config["collection_config"]["weekdays"],
                        "force_last_day": scenario_config["collection_config"].get(
                            "force_last_day", False
                        ),
                        "retry": scenario_config["collection_config"].get(
                            "retry", False
                        ),
                        "retry_count": scenario_config["collection_config"][
                            "retry_count"
                        ],
            ...
            if config["server_config"]["enable_secure_connection"]:
                secure_connection = {
                    "allow_insecure_connection": scenario_config["server_config"][
                        "secure_connection"
                    ].get("allow_insecure_connection", False),
                    "certificate_files_from_device": scenario_config["server_config"][
                        "secure_connection"
                    ].get("certificate_files_from_device", False),
                }

                # Add device certificate paths
                if (
                    "device_certificates"
                    in scenario_config["server_config"]["secure_connection"]
                ):
                    secure_connection["device_certificates"] = {
                        "certificate_authority_file": scenario_config["server_config"][
                            "secure_connection"
                        ]["device_certificates"]["certificate_authority_file"],
                        "client_certificate": scenario_config["server_config"][
                            "secure_connection"
                        ]["device_certificates"]["client_certificate"],
                        "client_private_keyfile": scenario_config["server_config"][
                            "secure_connection"
                        ]["device_certificates"]["client_private_keyfile"],
                    }
    ...
    async def cleanup(self):
        ...
        try:
            ...
            if deleted:
                ...
                logger.warning(
                    "Cleanup completed: No interfaces deleted or encountered issues."
                )
        except Exception as e:
