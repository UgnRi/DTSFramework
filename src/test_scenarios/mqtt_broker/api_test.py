...
class MQTTBrokerAPITest(BaseAPITest):
        ...
        try:
            ...
            if "miscellaneous" in config:
                ...
                if "persistence" in misc_config:
                    mqtt_config["data"][0]["persistence"] = (
                        "1" if misc_config["persistence"] else "0"
                    )
                if "max_queued_messages" in misc_config:
                    mqtt_config["data"][0]["max_queued_messages"] = str(
                        misc_config["max_queued_messages"]
                    )
                if "maximum_packet_size" in misc_config:
                    mqtt_config["data"][0]["max_packet_size"] = str(
                        misc_config["maximum_packet_size"]
                    )
            ...
            try:
                ...
                minimal_config = {
                    "data": {
                        "enabled": "1",
                        "anonymous_access": (
                            "1" if config.get("anonymous_access", True) else "0"
                        ),
    ...
    def _build_mqtt_config(
        ...
        try:
            ...
            if "anonymous_access" in config:
                config_item["anonymous_access"] = (
                    "1" if config["anonymous_access"] else "0"
                )
    ...
    def _configure_security_settings(
        self, mqtt_config: Dict[str, Any], security_config: Dict[str, Any]
    ):
        ...
        try:
            ...
            if "certificates" in security_config:
                ...
                if "require_certificate" in cert_config:
                    mqtt_config["require_certificate"] = (
                        "1" if cert_config["require_certificate"] else "0"
                    )
    ...
    def _configure_misc_settings(
        self, mqtt_config: Dict[str, Any], misc_config: Dict[str, Any]
    ):
        ...
        try:
            ...
            if "allow_anonymous" in misc_config:
                mqtt_config["anonymous_access"] = (
                    "1" if misc_config["allow_anonymous"] else "0"
                )

            # Configure max queued messages
            if "max_queued_messages" in misc_config:
                mqtt_config["max_queued_messages"] = str(
                    misc_config["max_queued_messages"]
                )
        ...
        try:
            ...
            if "port" in config:
                ...
                if expected_port not in actual_ports:
                    logger.error(
                        f"MQTT port mismatch. Expected: {expected_port}, Got: {actual_ports}"
                    )
                    return False
            ...
            if "anonymous_access" in config:
                expected_access = str(
                    self.format_bool_value(config["anonymous_access"])
                )
                actual_access = current_item.get("anonymous_access")
                if actual_access != expected_access:
                    logger.error(
                        f"MQTT anonymous access mismatch. Expected: {expected_access}, Got: {actual_access}"
                    )
                    return False

            process_info = await self.api_request("get", "mqtt/broker/config")
            if isinstance(process_info, dict) and isinstance(
                process_info.get("data"), list
            ):
