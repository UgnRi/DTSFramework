...
class MQTTBrokerSSHTest(BaseSSHTest):
    ...
    async def execute(self):
        """Execute MQTT broker configuration"""
        try:
            # Basic settings
            ...
            self.logger.info("MQTT broker configuration executed successfully")

        except Exception as e:
            self.logger.error(f"Failed to execute MQTT broker configuration: {str(e)}")
            raise
        ...
        try:
            ...
            for config_key, (uci_key, value_type) in settings_map.items():
                if config_key in misc_config:
                    ...
                    formatted_value = self.format_bool_value(value) if value_type is bool else str(value)
                    await self.execute_uci_command(
            ...
            self.logger.error(f"Failed to configure MQTT miscellaneous settings: {str(e)}")
            raise
    ...
    async def _configure_basic_settings(self):
        ...
        try:
            ...
            # Configure port if specified
            if 'port' in self.scenario_config:
                await self.ssh_client.execute_command(f'uci set mosquitto.mqtt.local_port={self.scenario_config["port"]}')
    ...
    async def _configure_security_settings(self):
        ...
        try:
            ...
            logger.error(f"Failed to configure MQTT security settings via SSH: {str(e)}")
            raise
    ...
    async def _configure_misc_settings(self):
        ...
        try:
            ...
            logger.error(f"Failed to configure MQTT miscellaneous settings via SSH: {str(e)}")
            raise
...
class DataToServerSSHTest(BaseTestScenario):
    ...
    async def configure_dts(self):
        ...
        try:
            ...
            await self.ssh_client.execute_command(f'uci set data_sender.1.name="{instance_name}"')
    ...
    async def _configure_data_collection(self):
        ...
        try:
            ...
            await self.ssh_client.execute_command(f'uci set data_sender.1.type="{data_config["type"]}"')
            ...
            if 'type_settings' in data_config:
                ...
                for key, value in type_settings.items():
                    ...
                    await self.ssh_client.execute_command(f'uci set data_sender.1.{key}="{value}"')

            # Set format type
            await self.ssh_client.execute_command(f'uci set data_sender.1.format_type="{data_config["format_type"]}"')

            # Configure values to collect
            if 'values' in data_config:
                values_str = ' '.join(data_config['values'])
                await self.ssh_client.execute_command(f'uci set data_sender.1.values="{values_str}"')
    ...
    async def _configure_scheduler(self):
        ...
        try:
            ...
            await self.ssh_client.execute_command('uci set data_sender.1.timer="scheduler"')
            await self.ssh_client.execute_command(f'uci set data_sender.1.day_time="{scheduler_config["day_time"]}"')
            await self.ssh_client.execute_command(f'uci set data_sender.1.interval_type="{scheduler_config["interval_type"]}"')

            # Set month days
            if 'month_day' in scheduler_config:
                month_days = ' '.join(map(str, scheduler_config['month_day']))
                await self.ssh_client.execute_command(f'uci set data_sender.1.month_day="{month_days}"')

            # Set weekdays
            if 'weekdays' in scheduler_config:
                weekdays = ' '.join(scheduler_config['weekdays'])
                await self.ssh_client.execute_command(f'uci set data_sender.1.weekdays="{weekdays}"')

            # Set additional options
            if scheduler_config.get('force_last_day'):
                await self.ssh_client.execute_command('uci set data_sender.1.force_last_day=1')

            if scheduler_config.get('retry'):
                await self.ssh_client.execute_command('uci set data_sender.1.retry=1')
                await self.ssh_client.execute_command(f'uci set data_sender.1.retry_count={scheduler_config["retry_count"]}')
                await self.ssh_client.execute_command(f'uci set data_sender.1.timeout={scheduler_config["timeout"]}')
    ...
    async def _configure_period(self):
        ...
        try:
            ...
            # Set period
            await self.ssh_client.execute_command(f'uci set data_sender.1.period={period_config["period"]}')
    ...
    async def _configure_server(self):
        ...
        try:
            ...
            await self.ssh_client.execute_command(f'uci set data_sender.2.mqtt_host="{server_config["server_address"]}"')
            await self.ssh_client.execute_command(f'uci set data_sender.2.mqtt_port={server_config["port"]}')
            await self.ssh_client.execute_command(f'uci set data_sender.2.mqtt_keepalive={server_config["keepalive"]}')
            await self.ssh_client.execute_command(f'uci set data_sender.2.mqtt_topic="{server_config["topic"]}"')
            await self.ssh_client.execute_command(f'uci set data_sender.2.mqtt_client_id="{server_config["client_id"]}"')
            await self.ssh_client.execute_command(f'uci set data_sender.2.mqtt_qos={server_config["QoS"]}')

            # Configure secure connection if enabled
            if server_config.get('enable_secure_connection'):
                await self.ssh_client.execute_command('uci set data_sender.2.mqtt_tls=1')
                
                secure_config = server_config['secure_connection']
                
                if secure_config.get('allow_insecure_connection'):
                    await self.ssh_client.execute_command('uci set data_sender.2.mqtt_insecure=1')

                if secure_config.get('certificate_files_from_device'):
                    await self.ssh_client.execute_command('uci set data_sender.2.mqtt_device_files=1')
            ...
            if server_config.get('use_credentials'):
                await self.ssh_client.execute_command('uci set data_sender.2.mqtt_use_credentials=1')
                await self.ssh_client.execute_command(f'uci set data_sender.2.mqtt_username="{server_config["username"]}"')
                await self.ssh_client.execute_command(f'uci set data_sender.2.mqtt_password="{server_config["password"]}"')
