    ...
    async def logout(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"Failed to capture button information: {str(btn_error)}")
                ...
                try:
                    ...
                    if dropdown_count > 0:
                        ...
                        try:
                            ...
                            self.logger.info(f"Dropdown button - Visible: {is_visible}, Enabled: {is_enabled}")
                        except Exception as detail_error:
                            self.logger.error(f"Failed to get button details: {str(detail_error)}")
                        
                        try:
                            await first_button.click(timeout=5000)
                            self.logger.info(f"Successfully clicked dropdown with selector: {selector}")
                            ...
                            self.logger.error(f"Failed to click dropdown with {selector}: {str(click_error)}")
                
                except Exception as selector_error:
                    self.logger.error(f"Error with selector {selector}: {str(selector_error)}")
            ...
            if not dropdown_clicked:
                ...
                with open('dropdown_error_content.html', 'w', encoding='utf-8') as f:
                ...
                try:
                    ...
                    if logout_count > 0:
                        ...
                        try:
                            is_visible = await first_button.is_visible()
                            is_enabled = await first_button.is_enabled()
                            self.logger.info(f"Logout button - Visible: {is_visible}, Enabled: {is_enabled}")
                        except Exception as detail_error:
                            self.logger.error(f"Failed to get logout button details: {str(detail_error)}")
                        
                        try:
                            await first_button.click(timeout=5000)
                            self.logger.info(f"Successfully clicked logout with selector: {selector}")
                            logout_clicked = True
                            ...
                            self.logger.error(f"Failed to click logout with {selector}: {str(click_error)}")
            ...
            if not logout_clicked:
                ...
                # Capture page content
                page_content = await self.page.content()
                with open('logout_error_content.html', 'w', encoding='utf-8') as f:
                    f.write(page_content)
