...
class DTSPage(BasePage):
    ...
    async def configure_dts(self, config):
        ...
        try:
            ...
            if 'data_config' in config:
                ...
                await self.page.wait_for_timeout(1000)  # Increased wait after data collection
            ...
            if 'collection_config-scheduler' in config:
                ...
                if 'timer' in collection_config and collection_config['timer'] == 'scheduler':
                    ...
                    await self._configure_scheduler({
                        ...
                        'force_last_day': collection_config.get('force_last_day', False),
                ...
                await self.page.wait_for_timeout(1000)  # Increased wait after collection timing
            ...
            if 'format_type' in data_config:
                ...
                if data_config['format_type'] == 'Custom':
                    ...
                    if 'lua_format_script' in data_config:
                        await self._set_lua_format_script(data_config['lua_format_script'])
        ...
        try:
            ...
            if data_type == "Base":
                ...
                if 'data_filtering' in type_settings:
                    filtering_selector = self.page.locator('[test-id="selectstate-bl_filter selectedid-all"]')
                    await filtering_selector.click()
                    filtering_option =  self.page.get_by_role("option", name=type_settings['data_filtering'], exact=True)
                ...
                if 'device_name' in type_settings:
                    device_input = self.page.locator('[test-id="input-bl_filter_name_0"]')
                ...
                if 'send_as_object' in type_settings:
                    send_as_object_switch = self.page.locator('div[test-id="switch-bl_object"] >> visible=true')
                ...
                if 'data_filtering' in type_settings:
                    filtering_selector = self.page.locator('[test-id="input-impulse_counter_filter"]')
                    await filtering_selector.click()
                    filtering_option =  self.page.get_by_role("option", name=type_settings['data_filtering'], exact=True)
                    await filtering_option.wait_for(state='visible', timeout=30000)
                    await filtering_option.click()
                ...
                if 'impulse_counter_pin' in type_settings:
                    pin_selector = self.page.locator('[test-id="multiselect-impulse_counter_filter_pin"]')
                    await pin_selector.click()                    
                    pin_option =  self.page.get_by_role("option", name=type_settings['impulse_counter_pin'], exact=True)
                ...
                if 'invert_filter' in type_settings:
                    invert_switch = self.page.locator('div[test-id="switch-impulse_counter_filter_invert"] >> visible=true')
                ...
                if 'max_segment_count' in type_settings:
                    segment_count = self.page.locator('[test-id="input-impulse_counter_segments"]')
                    await segment_count.fill(str(type_settings['max_segment_count']))
                
                if 'send_as_object' in type_settings and type_settings.get('max_segment_count', 1) == 1:
                    send_as_object_switch = self.page.locator('div[test-id="switch-impulse_counter_object"] >> visible=true')
                    await send_as_object_switch.wait_for(state='visible')
                    toggle = send_as_object_switch.locator('div[tabindex="0"]')
                    await toggle.click()
                    await self.page.wait_for_timeout(500)
                ...
                if 'lua_script_location' in type_settings:
                    script_input = self.page.locator('[test-id="upload-input-lua_script"]')
                    await script_input.set_input_files(type_settings['lua_script_location'])

            elif data_type == "Mobile usage":
                if 'data_period' in type_settings:
                    period_selector = self.page.locator('[test-id="selectwrapper-mdc_period"]')
                    await period_selector.click()                    
                    period_option =  self.page.get_by_role("option", name=type_settings['data_period'], exact=True)
                    await period_option.wait_for(state='visible', timeout=30000)
                    await period_option.click()

                if 'current' in type_settings:
                    current_switch = self.page.locator('div[test-id="switch-mdc_current"] >> visible=true')
                ...
                if 'SIM_number' in type_settings:
                    sim_selector = self.page.locator('[test-id="selectwrapper-mdc_sim"]')
                    await sim_selector.click()                    
                    sim_option =  self.page.get_by_role("option", name=type_settings['SIM_number'], exact=True)
                ...
                if 'data_filtering' in type_settings:
                    filtering_selector = self.page.locator('[test-id="input-modbus_filter"]')
                    await filtering_selector.click()
                    filtering_option =  self.page.get_by_role("option", name=type_settings['data_filtering'], exact=True)
                    await filtering_option.wait_for(state='visible', timeout=30000)
                    await filtering_option.click()
                ...
                if 'server_ip' in type_settings:
                    ip_input = self.page.locator('[test-id="input-modbus_filter_server_ip_0"]')
                    await ip_input.fill(type_settings['server_ip'])

                if 'server_id' in type_settings:
                    id_input = self.page.locator('[test-id="input-modbus_filter_server_id_0"]')
                    await id_input.fill(type_settings['server_id'])

                if 'request_name' in type_settings:
                    request_name_input = self.page.locator('[test-id="input-modbus_filter_request_0"]')
                    await request_name_input.fill(type_settings['request_name'])

                if 'segment_count' in type_settings:
                    segment_count = self.page.locator('[test-id="input-modbus_segments"]')
                    await segment_count.fill(str(type_settings['segment_count']))

                if 'send_as_object' in type_settings and type_settings.get('segment_count', 1) == 1:
                    send_as_object_switch = self.page.locator('div[test-id="switch-modbus_object" >> visible=true')
                    await send_as_object_switch.wait_for(state='visible')
                    toggle = send_as_object_switch.locator('div[tabindex="0"]')
                    await toggle.click()
                    await self.page.wait_for_timeout(500)
                ...
                if 'data_filtering' in type_settings:
                    filtering_selector = self.page.locator('[test-id="input-modbus_alarm_filter"]')
                    await filtering_selector.click()
                    filtering_option =  self.page.get_by_role("option", name=type_settings['data_filtering'], exact=True)
                    await filtering_option.wait_for(state='visible', timeout=30000)
                    await filtering_option.click()
                ...
                if 'server_id' in type_settings:
                    server_id_input = self.page.locator('[test-id="input-modbus_alarm_filter_server_id_0"]')
                    await server_id_input.fill(str(type_settings['server_id']))


                if 'alarm_id' in type_settings:
                    alarm_id_input = self.page.locator('[test-id="input-modbus_alarm_filter_alarm_id_0"]')
                    await alarm_id_input.fill(str(type_settings['alarm_id']))

                if 'register_number' in type_settings:
                    register_number_input = self.page.locator('[test-id="input-modbus_alarm_filter_register_0"]')
                    await register_number_input.fill(str(type_settings['register_number']))
                ...
                if 'data_filtering' in type_settings:
                    ...
                    filtering_selector = self.page.locator('[test-id="input-wifi_filter"]')
                    await filtering_selector.click()
                    await self.page.wait_for_timeout(500)  # Wait for dropdown to open
                    
                    # More specific selector using the listbox context
                    option = self.page.locator(f'[test-id="selectwrapper-wifi_filter-listbox"] [test-id="selectoption-{mapped_type}"]')
                    ...
                    if 'hostname' in type_settings:
                        for i, hostname in enumerate(type_settings['hostname']):
                            # Input the hostname
                            hostname_input = self.page.locator(f'[test-id="input-wifi_filter_name_{i}"]')
                            ...
                            if i < len(type_settings['hostname']) - 1:
                                add_button = self.page.locator(f'[test-id="listadd-wifi_filter_signal_{i}"]')
                                await add_button.click()
                                await self.page.wait_for_timeout(500)  # Wait for new field to appear
                    
                    # Handle MAC address entries
                    if 'mac_address' in type_settings:
                        for i, address in enumerate(type_settings['mac_address']):
                            # Input the MAC address
                            address_input = self.page.locator(f'[test-id="input-wifi_filter_mac_{i}"]')
                    ...
                    if 'signal_strength' in type_settings:
                        for i, strength in enumerate(type_settings['signal_strength']):
                            ...
                            strength_input = self.page.locator(f'[test-id="input-wifi_filter_signal_{i}"]')
                    ...
                    if 'segment_count' in type_settings:
                        segment_count = self.page.locator('[test-id="input-wifi_segments"]')
                        await segment_count.fill(str(type_settings['segment_count']))

                    if 'send_as_object' in type_settings and segment_count == 1:
                        send_object_switch = self.page.locator('div[test-id="switch-wifi_object"] >> visible=true')
                        await send_object_switch.wait_for(state='visible')
                        toggle = send_object_switch.locator('div[tabindex="0"]')
                        await toggle.click()

            logger.info(f"Successfully configured type-specific settings for {data_type}")

        except Exception as e:
            logger.error(f"Failed to configure type-specific settings for {data_type}: {str(e)}")
            raise
    ...
    async def _configure_mqtt_input_settings(self, mqtt_settings):
        ...
        try:
            ...
            await self.page.locator(f'[test-id="selectoption-{mqtt_settings["QoS"]}"]').click()

            # Secure connection settings
            if mqtt_settings.get('enable_secure_connection'):
                secure_conn_switch = self.page.locator('div[test-id="switch-mqtt_in_tls"] >> visible=true')
                ...
                # TLS settings
                tls_type_dropdown = self.page.locator('[test-id="input-mqtt_in_tls_type"]')
                ...
                if secure_config.get('allow_insecure_connection'):
                    insecure_switch = self.page.locator('div[test-id="switch-mqtt_in_insecure"]  >> visible=true')
                ...
                if secure_config.get('certificate_files_from_device'):
                    device_files_switch = self.page.locator('div[test-id="switch-mqtt_device_files"] >> visible=true')
                    ...
                    if secure_config.get('device_certificates', {}).get('certificate_authority_file'):
                        # Click CA file dropdown
                        ca_dropdown = self.page.locator('[test-id="input-mqtt_in_cafile"]')
                        await ca_dropdown.click()
                        await self.page.wait_for_timeout(500)
                        
                        # Click the option within the visible dropdown list
                        dropdown_list = self.page.locator('[test-id="selectwrapper-mqtt_in_cafile-listbox"]')
                        ca_option = dropdown_list.get_by_text(secure_config["device_certificates"]["certificate_authority_file"])
                        await ca_option.click()
                        await self.page.wait_for_timeout(500)

                    if secure_config.get('device_certificates', {}).get('client_certificate'):
                        # Click client cert dropdown  
                        cert_dropdown = self.page.locator('[test-id="input-mqtt_in_certfile"]')
                        await cert_dropdown.click()
                        await self.page.wait_for_timeout(500)

                        dropdown_list = self.page.locator('[test-id="selectwrapper-mqtt_in_certfile-listbox"]')
                        cert_option = dropdown_list.get_by_text(secure_config["device_certificates"]["client_certificate"])
                        await cert_option.click()
                        await self.page.wait_for_timeout(500)

                    if secure_config.get('device_certificates', {}).get('client_private_keyfile'):
                        # Click private key dropdown
                        key_dropdown = self.page.locator('[test-id="input-mqtt_in_keyfile"]')
                        await key_dropdown.click() 
                        await self.page.wait_for_timeout(500)

                        dropdown_list = self.page.locator('[test-id="selectwrapper-mqtt_in_keyfile-listbox"]')
                        key_option = dropdown_list.get_by_text(secure_config["device_certificates"]["client_private_keyfile"])
                        await key_option.click()
                        await self.page.wait_for_timeout(500)
                else:
                    # Upload certificate files
                    if secure_config.get('certificate_authority_file'):
                        ca_input = self.page.locator('[test-id="upload-input-mqtt_in_cafile"]')
                        await ca_input.set_input_files(secure_config['certificate_authority_file'])

                    if secure_config.get('client_certificate'):
                        cert_input = self.page.locator('[test-id="upload-input-mqtt_in_certfile"]')
                        await cert_input.set_input_files(secure_config['client_certificate'])

                    if secure_config.get('client_private_keyfile'):
                        key_input = self.page.locator('[test-id="upload-input-mqtt_in_keyfile"]')
                        await key_input.set_input_files(secure_config['client_private_keyfile'])
    ...
    async def _set_lua_format_script(self, script_path):
        ...
        try:
            script_input = self.page.locator('[test-id="upload-input-lua_format_script"]')
    ...
    async def _set_collection_name(self, name):
        ...
        try:
            ...
            collection_name = self.page.locator('#section-data-configuration input[test-id="input-name"]')
            ...
            # Type the new name
            await collection_name.type(name, delay=50)  # Add slight delay between keystrokes
    ...
    async def _set_data_type(self, data_type):
        ...
        try:
            ...
            dropdown_wrapper = self.page.locator('#section-data-configuration [test-id="selectwrapper-plugin"]')
            ...
            option_elements = await self.page.locator(f'[test-id="selectoption-{mapped_type}"]').all()
            ...
            if data_type.lower() not in dropdown_text.lower():
                raise Exception(f"Selection verification failed. Expected: {data_type}, Got: {dropdown_text}")
    ...
    async def _set_format_type(self, format_type):
        ...
        try:
            ...
            format_dropdown = self.page.locator('#section-data-configuration [test-id="selectwrapper-format"]')
    ...
    async def _configure_values(self, values):
        ...
        try:
            ...
            values_button = self.page.locator('#section-data-configuration [test-id="multiselect-members"]')
    ...
    async def _configure_server(self, server_config):
        ...
        try:
            ...
            plugin_dropdown = self.page.locator('#section-server-configuration [test-id="input-plugin"]')
            ...
            server_address = self.page.locator('#section-server-configuration [test-id="input-mqtt_host"]')
            ...
            port_field = self.page.locator('#section-server-configuration [test-id="input-mqtt_port"]')
            ...
            keepalive_field = self.page.locator('#section-server-configuration [test-id="input-mqtt_keepalive"]')
            ...
            topic_field = self.page.locator('#section-server-configuration [test-id="input-mqtt_topic"]')
            await topic_field.fill(server_config.get('topic', ''))

            # Set client ID
            client_id_field = self.page.locator('#section-server-configuration [test-id="input-mqtt_client_id"]')
            ...
            mqtt_option = self.page.get_by_role("option", name=str(server_config['QoS']), exact=True)
            await mqtt_option.wait_for(state='visible', timeout=30000)
            await mqtt_option.click()
            ...
            if server_config.get('enable_secure_connection'):
                ...
                secure_conn_switch = self.page.locator('#section-server-configuration div[test-id="switch-mqtt_tls"] >>visible=true')
                await secure_conn_switch.wait_for(state='visible')
                toggle = secure_conn_switch.locator('div[tabindex="0"]')
                await toggle.click()
                await self.page.wait_for_timeout(500)
                ...
                tls_type_dropdown = self.page.locator('[test-id="selectwrapper-mqtt_tls_type"]')
                await tls_type_dropdown.click()
                await self.page.wait_for_timeout(500)
                
                cert_option = self.page.locator('[test-id="selectwrapper-mqtt_tls_type-listbox"] [test-id="selectoption-cert"]')
                await cert_option.click()
                await self.page.wait_for_timeout(500)

                secure_config = server_config.get('secure_connection', {})

                insecure_switch = self.page.locator('div[test-id="switch-mqtt_insecure"] >> visible=true')
                await insecure_switch.wait_for(state='visible')  # Make sure switch is visible
                ...
                if secure_config.get('certificate_files_from_device'):
                    if await device_files_switch.get_attribute('aria-checked') == 'false':
                        toggle = device_files_switch.locator('div[tabindex="0"]')
                        await toggle.click()
                        await self.page.wait_for_timeout(500)
                    ...
                    if secure_config.get('device_certificates', {}).get('certificate_authority_file'):
                        # Click CA file dropdown
                        ca_dropdown = self.page.locator('[test-id="selectwrapper-mqtt_cafile"]')
                        await ca_dropdown.click()
                        await self.page.wait_for_timeout(500)
                        
                        # Click the option within the visible dropdown list
                        dropdown_list = self.page.locator('[test-id="selectwrapper-mqtt_cafile-listbox"]')
                        ca_option = dropdown_list.get_by_text(secure_config["device_certificates"]["certificate_authority_file"])
                        await ca_option.click()
                        await self.page.wait_for_timeout(500)
                    ...
                    if secure_config.get('device_certificates', {}).get('client_certificate'):
                        # Click client cert dropdown  
                        cert_dropdown = self.page.locator('[test-id="selectwrapper-mqtt_certfile"]')
                        await cert_dropdown.click()
                        await self.page.wait_for_timeout(500)

                        dropdown_list = self.page.locator('[test-id="selectwrapper-mqtt_certfile-listbox"]')
                        cert_option = dropdown_list.get_by_text(secure_config["device_certificates"]["client_certificate"])
                        await cert_option.click()
                        await self.page.wait_for_timeout(500)
                    ...
                    if secure_config.get('device_certificates', {}).get('client_private_keyfile'):
                        # Click private key dropdown
                        key_dropdown = self.page.locator('[test-id="selectwrapper-mqtt_keyfile"]')
                        await key_dropdown.click() 
                        await self.page.wait_for_timeout(500)

                        dropdown_list = self.page.locator('[test-id="selectwrapper-mqtt_keyfile-listbox"]')
                        key_option = dropdown_list.get_by_text(secure_config["device_certificates"]["client_private_keyfile"])
                        await key_option.click()
                        await self.page.wait_for_timeout(500)
                else:
                    if await device_files_switch.get_attribute('aria-checked') == 'true':
                        toggle = device_files_switch.locator('div[tabindex="0"]')
                        await toggle.click()
                        await self.page.wait_for_timeout(500)
                ...
                if secure_config.get('certificate_authority_file'):
                    ca_file_input = self.page.locator('[test-id="upload-input-mqtt_cafile"]')
                    await ca_file_input.set_input_files(secure_config['certificate_authority_file'])
                    await self.page.wait_for_timeout(500)

                if secure_config.get('client_certificate'):
                    cert_file_input = self.page.locator('[test-id="upload-input-mqtt_certfile"]')
                    await cert_file_input.set_input_files(secure_config['client_certificate'])
                    await self.page.wait_for_timeout(500)

                if secure_config.get('client_private_keyfile'):
                    key_file_input = self.page.locator('[test-id="upload-input-mqtt_keyfile"]')
                    await key_file_input.set_input_files(secure_config['client_private_keyfile'])
                    await self.page.wait_for_timeout(500)
            ...
            if server_config.get('use_credentials'):
                ...
                credentials_switch = self.page.locator('div[test-id="switch-mqtt_use_credentials"] >> visible=true')
    ...
    async def _configure_scheduler(self, scheduler_config):
        ...
        try:
            ...
            timer_dropdown = self.page.locator('#section-collection-configuration [test-id="selectwrapper-timer"]')
            ...
            # Set day time
            time_input = self.page.locator('#section-collection-configuration input[test-id="input-time_0"]')
            ...
            interval_dropdown = self.page.locator('#section-collection-configuration [test-id="selectwrapper-day_mode"]')
            ...
            interval_option = self.page.get_by_role('option').filter(
                has=self.page.get_by_text(scheduler_config.get('interval_type', ''), exact=True)
            )
            ...
            if scheduler_config['timer'] == 'period':
                ...
                for day in weekdays:
                    day_checkbox = self.page.locator(f'[test-id="checkbox-{day.lower()}"]')
            ...
            if 'month_day' in scheduler_config:
                ...
                # Open the dropdown
                month_days_button = self.page.locator('[test-id="multiselect-month_days"]')
                ...
                for value in scheduler_config.get('month_day', []):
                    try:
                        list_item = self.page.locator(f'#data_sender\\.collection\\.1\\.edit\\.1_month_days-select-{value}')
                        await list_item.wait_for(state='visible', timeout=5000)
                        label = list_item.locator('label')
                        await label.click()
                        await self.page.wait_for_timeout(200)
                    except Exception as value_error:
                        logger.warning(f"Error selecting month day {value}: {value_error}")
                
                # Close the dropdown by clicking the dropdown arrow
                try:
                    dropdown_arrow = self.page.locator('[test-id="multiselect-month_days"] .tlt-input-icon-right')
            ...
            try:
                ...
                if scheduler_config.get('force_last_day'):
                    force_last_day_switch = self.page.locator('[id="data_sender.collection.1.edit.1_last_day"]')
                ...
                if scheduler_config.get('retry'):
                    retry_switch = self.page.locator('[id="data_sender.collection.1.edit.1_retry"]')
            ...
            try: 
                if scheduler_config.get('retry_count'):
                    retry_count_field = self.page.locator('[id="data_sender.collection.1.edit.1_retry_count"]')
                ...
                if scheduler_config.get('timeout'):
                    timeout_field = self.page.locator('[id="data_sender.collection.1.edit.1_retry_timeout"]')
    ...
    async def _click_next(self):
        ...
        try:
            next_button = self.page.locator('div[test-id="modal-container"] button[test-id="button-next"]')
