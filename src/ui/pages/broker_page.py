...
import os
from src.backend.certificate_generator import generate_mqtt_certificates, prepare_mqtt_certificates
...
class BrokerPage(BasePage):
    ...
    async def configure_basic_settings(self, config):
        ...
        try:
            ...
            if 'remote_access' in config:
                remote_switch = self.page.locator('[id="mosquitto.mqtt.mqtt.mqtt_allow_ra"]')
                await remote_switch.wait_for(state='visible', timeout=30000)
                switch_container = self.page.locator('div[aria-checked]').filter(has=remote_switch)
                is_remote = await switch_container.get_attribute('aria-checked') == 'true'
    ...
    async def handle_switch(self, id_selector, test_id_selector, desired_state, description):
    ...
    async def configure_security(self, security_config):
            ...
            try:
                if not security_config:
                    logger.info("No security config provided, skipping security configuration")
                    return
    ...
    async def _configure_tls_ssl(self, enabled):
        ...
        try:
            ssl_switch = self.page.locator('[id="mosquitto.mqtt.mqtt.mqtt_use_tls_ssl"]')
            await ssl_switch.wait_for(state='visible', timeout=30000)
            
            switch_container = self.page.locator('div[test-id="switch-use_tls_ssl"]')
            is_ssl_enabled = await switch_container.get_attribute('aria-checked') == 'true'
    ...
    async def _configure_psk(self, cert_config):
        ...
        try:
            if 'pre-shared-key' in cert_config:
                await self.page.fill('[test-id="input-psk"]', cert_config['pre-shared-key'])
            if 'identity' in cert_config:
                await self.page.fill('[test-id="input-identity"]', cert_config['identity'])
            await self.page.wait_for_timeout(500)
        except Exception as e:
    ...
    async def _configure_cert_based(self, cert_config):
        ...
        try:
            ...
            if 'certificate_files_from_device' in cert_config:
                await self._set_device_certificates(cert_config['certificate_files_from_device'])
        ...
        try:
            ...
            for cert_type, selectors in cert_types.items():
                ...
                try:
                    ...
                    option = self.page.get_by_role("option", name=target_filename, exact=True)
                    ...
                    try:
                        ...
                        logger.error(f"Available options for {cert_type}: {available_options}")
    ...
    async def _select_from_dropdown(self, dropdown_id, target_value):
        ...
        try:
            ...
            logger.error(f"Failed to select {target_value} from dropdown {dropdown_id}: {str(e)}")
            return False
    ...
    async def _select_certificate_from_dropdown(self, dropdown_id, filename):
        ...
        try:
            ...
            logger.error(f"Failed to select certificate {filename} from dropdown: {str(e)}")
            return False

    async def _set_require_certificate(self, enabled):
        """Set require certificate setting"""
        try:
            req_cert_switch = self.page.locator('[id="mosquitto.mqtt.mqtt.mqtt_require_certificate"]')
            await req_cert_switch.wait_for(state='visible', timeout=30000)
            
            req_cert_container = self.page.locator('div[test-id="switch-require_certificate"]')
            is_req_cert = await req_cert_container.get_attribute('aria-checked') == 'true'
    ...
    async def _set_device_certificates(self, enabled):
        ...
        try:
            device_cert_switch = self.page.locator('[id="mosquitto.mqtt.mqtt.mqtt_device_sec_files"]')
            await device_cert_switch.wait_for(state='visible', timeout=30000)
            
            device_cert_container = self.page.locator('div[test-id="switch-device_sec_files"]')
            is_device_cert = await device_cert_container.get_attribute('aria-checked') == 'true'
    ...
    async def _upload_hidden_file(self, file_path, input_selector, button_selector, label_id=None):
        """
        ...
        try:
            ...
            file_name_display = self.page.locator(f'[test-id="upload-file-{input_selector.split("-")[-1]}"]')
            await file_name_display.wait_for(state='attached', timeout=5000)
            
            display_text = await file_name_display.inner_text()
            if file_name not in display_text and "or drag and drop your file here" in display_text:
    ...
    async def configure_miscellaneous(self, misc_config):
        ...
        try:
            if not misc_config:
                logger.info("No miscellaneous configuration provided, skipping configuration")
                return
    ...
    async def _handle_acl_file(self, acl_config):
        ...
        try:
            if not acl_config or 'acl_file_location' not in acl_config or 'rules' not in acl_config:
    ...
    async def _handle_password_file(self, password_config):
        ...
        try:
            if not password_config or 'password_file_location' not in password_config or 'users' not in password_config:
                logger.warning("Insufficient password file configuration")
                return

            # Check if password file is already uploaded
            password_file_label = self.page.locator('label[id="mqtt_password_file-label"]')
            ...
            if password_file_path and os.path.exists(password_file_path):
                ...
                logger.info(f"Successfully uploaded password file: {password_file_path}")
            else:
    ...
    async def _set_persistence(self, enabled):
        ...
        try:
            persistence_switch = self.page.locator('[id="mosquitto.mqtt.mqtt.mqtt_persistence"]')
            ...
            is_persistent = await switch_container.get_attribute('aria-checked') == 'true'
    ...
    async def _set_anonymous_access(self, enabled):
        ...
        try:
            anonymous_switch = self.page.locator('[id="mosquitto.mqtt.mqtt.mqtt_anonymous_access"]')
            ...
            is_anonymous = await anonymous_container.get_attribute('aria-checked') == 'true'
        ...
        try:
            ...
            for config_key, path in [
                ...
                if path:
                    ...
                    logger.info(f"Uploading {config_key} using test-id {test_id}, path: {path}")
    ...
    async def handle_certificate_selection(self, cert_config):
        ...
        try:
            ...
            for config_key, mapping in cert_mapping.items():
                ...
                options = self.page.locator(f'[test-id="{mapping["wrapper"]}-listbox"] [test-id="options-wrapper"] li div.truncate')
