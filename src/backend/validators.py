...
class WirelessValidator:
    ...
    async def validate_mqtt_message(self, server_config, expected_topic, mqtt_server, timeout=60):
        """
        ...
        try:
            ...
            try:
                ...
                result = await asyncio.wait_for(
                    ...
                    timeout=timeout + 5  # Add a small buffer to subprocess timeout
                )
    ...
    async def validate_ap_config(self, mqtt_config=None, dts_config=None):
        ...
        try:
            ...
            return {
                'success': False,
                'details': 'Validation process timed out'
            }
            ...
            for line in lines:
                if '.name=' in line and instance_name in line:
                    ...
                    if len(path_parts) >= 3 and path_parts[0] == 'data_sender':
                        ...
                        logger.info(f"Found DTS section with name '{instance_name}': {section_id}")
                        ...
                        for type_line in lines:
                            if f"data_sender.{section_id}=" in type_line:
                                ...
                                logger.info(f"Section {section_id} is of type: {section_type}")
                                break
                        ...
                        for ref_line in lines:
                            if f".input={section_id}" in ref_line:
                                ...
                                logger.info(f"Found collection section {ref_id} referencing input {section_id}")
                            elif f".output={section_id}" in ref_line:
                                ref_path = ref_line.split('=')[0]
                                ref_id = ref_path.split('.')[1]
                                found_sections['collection'] = ref_id
                                logger.info(f"Found collection section {ref_id} referencing output {section_id}")
                                ...
                                input_line = next((l for l in lines if f"data_sender.{coll_id}.input=" in l), None)
                                if input_line:
                                    input_id = input_line.split('=')[1].strip("'\"")
                                    found_sections['input'] = input_id
                                
                                output_line = next((l for l in lines if f"data_sender.{coll_id}.output=" in l), None)
                                if output_line:
    ...
    async def _cleanup_configuration(self, dts_config=None, clean_all=False):
        ...
        try:
                ...
                for line in lines:
                    if '=' in line:
                        ...
                        if len(parts) >= 2 and parts[0] == 'data_sender':
                            section_id = parts[1]
                            section_type = line.split('=')[1] if '=' in line else "unknown"
                            if section_id != 'settings' and section_id not in sections_to_delete:
                                sections_to_delete.add(section_id)
                                logger.info(f"Added section {section_id} (type: {section_type}) for deletion")
            else:
                # Extract the nested config if present
                dts_config = dts_config.get('config', dts_config) if dts_config else None
                
                if not dts_config:
                    logger.warning("No DTS configuration provided for cleanup, deleting all input sections")
                    ...
                    for line in lines:
                        if '=' in line:
                            ...
                            if len(parts) >= 2 and parts[0] == 'data_sender':
                                section_id = parts[1]
                                section_type = line.split('=')[1] if '=' in line else "unknown"
                                if (section_type == 'input' or 'input' in line) and section_id not in sections_to_delete:
                                    sections_to_delete.add(section_id)
                                    logger.info(f"Added input section {section_id} for deletion")
                else:
                    # Get the instance name
                    instance_name = dts_config.get('instanceName', 'test_instance')
                    logger.info(f"Cleaning up configuration for instance: '{instance_name}'")
                    ...
                    for line in lines:
                        if '.name=' in line and f"'{instance_name}'" in line:
                            ...
                            sections_to_delete.add(section_id)
                            logger.info(f"Found section {section_id} with name '{instance_name}'")
                    ...
                    for line in lines:
                        if '.input=' in line or '.output=' in line:
                            ...
                            sections_to_delete.add(section_id)
                            logger.info(f"Found reference from section {section_id} to section {ref_id}")
                    ...
                    for line in lines:
                        if ('=input' in line or '.plugin=' in line) and 'input' in line.lower():
                            section_path = line.split('=')[0]
                            section_id = section_path.split('.')[1]
                    ...
                    if not sections_to_delete:
                        logger.warning(f"No sections found for '{instance_name}', deleting all input/collection sections")
                        for line in lines:
                            if '=input' in line or '=collection' in line:
                                section_path = line.split('=')[0]
                                section_id = section_path.split('.')[1]
                                sections_to_delete.add(section_id)
                                logger.info(f"Added section {section_id} for deletion (fallback)")
            ...
            for section_id in sections_to_delete:
                section_type = await self.ssh_client.execute_command(f'uci get data_sender.{section_id} 2>/dev/null || echo "unknown"')
                section_type = section_type.strip()
                await self.ssh_client.execute_command(f'uci delete data_sender.{section_id}')
    ...
    async def _validate_ap_config(self, mqtt_config=None, dts_config=None):
        ...
        try:
            ...
            mqtt_config = mqtt_config.get('config', mqtt_config) if mqtt_config else None
            dts_config = dts_config.get('config', dts_config) if dts_config else None
            ...
            if mqtt_config:
                try:
                    ...
                    mqtt_port_value = mqtt_port.split('=')[1].strip("'\"") if '=' in mqtt_port else ""
                    ...
                    if not success:
                        ...
                        if not port_matches:
                            failed_checks.append(f"port (expected {expected_port}, got {mqtt_port_value})")
            ...
            if dts_config:
                try:
                    ...
                    if not sections or 'collection' not in sections:
                        logger.error(f"No matching DTS sections found for instance '{instance_name}'")
                        ...
                        if timer_type.strip() == "period":
                            ...
                            logger.info(f"Data to Server Scheduler Check: {dts_scheduler}")
                        ...
                        expected_server = server_config.get('server_address', 'test.mosquitto.org')
                        expected_topic = server_config.get('topic', 'test/topic')
                        expected_client_id = str(server_config.get('client_id', 'test_client'))
                        expected_qos = str(server_config.get('QoS', '0'))

                                                
                        # Parse actual values
                        actual_server = mqtt_server.split('=')[1].strip("'\"") if '=' in mqtt_server else ""
                        actual_topic = mqtt_topic.split('=')[1].strip("'\"") if '=' in mqtt_topic else ""
                        actual_client_id = mqtt_client_id.split('=')[1].strip("'\"") if '=' in mqtt_client_id else ""
                        actual_qos = mqtt_qos.split('=')[1].strip("'\"") if '=' in mqtt_qos else ""
                        ...
                        dts_name_value = dts_instance.split('=')[1].strip("'\"") if '=' in dts_instance else ""
                        dts_enabled_value = dts_enabled.split('=')[1].strip("'\"") if '=' in dts_enabled else ""
                        ...
                        success = all([
                            ...
                            actual_client_id == expected_client_id,  # Client ID matches
                        ...
                        results['data_to_server'] = {
                            ...
                            'details': {
                                ...
                                'mqtt_client_id_correct': actual_client_id == expected_client_id,
                        ...
                        if not success:
                            ...
                            if dts_name_value != instance_name:
                                failed_checks.append(f"instanceName (expected '{instance_name}', got '{dts_name_value}')")
                            if dts_enabled_value != "1":
                                failed_checks.append("DTS not enabled")
                            if actual_server != expected_server:
                                failed_checks.append(f"server (expected '{expected_server}', got '{actual_server}')")
                            if actual_topic != expected_topic:
                                failed_checks.append(f"topic (expected '{expected_topic}', got '{actual_topic}')")
                            if actual_client_id != expected_client_id:
                                failed_checks.append(f"client_id (expected '{expected_client_id}', got '{actual_client_id}')")
                            if actual_qos != expected_qos:
                                failed_checks.append(f"QoS (expected '{expected_qos}', got '{actual_qos}')")
                            if not mqtt_message_valid:
                                failed_checks.append("no MQTT message received")
                            
                            results['data_to_server']['details']['failures'] = failed_checks
            ...
            overall_success = (
                (not mqtt_config or results['mqtt_broker']['success']) and
                (not dts_config or results['data_to_server']['success'])
            )
            ...
            if mqtt_config:
                ...
                if not results['mqtt_broker']['success'] and isinstance(results['mqtt_broker']['details'], dict) and 'failures' in results['mqtt_broker']['details']:
                    logger.info(f"MQTT Broker Failures: {results['mqtt_broker']['details']['failures']}")
            if dts_config:
                logger.info(f"DTS Success: {results['data_to_server']['success']}")
                if not results['data_to_server']['success'] and isinstance(results['data_to_server']['details'], dict) and 'failures' in results['data_to_server']['details']:
                    logger.info(f"DTS Failures: {results['data_to_server']['details']['failures']}")
